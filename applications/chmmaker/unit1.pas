...
type
  ...
  TCHMForm = class(TForm)
    ...
    AddAllBtn: TButton;
    CreateBinaryIndexCkeckBox: TCheckBox;
    ...
    if FindFirst(ADir+'*', faAnyFile or faDirectory, SearchRec) = 0 then begin
      repeat
        if (SearchRec.Attr and faDirectory) <> 0 then begin
          FPos := Pos('.', SearchRec.Name);
          if (FPos = 0) or (FPos > 2)  then begin
...
begin
  ...
  Proc.CommandLine := '/home/andrew/programming/lazarus/components/chmhelp/lhelp/lhelp '+ ChmFileNameEdit.FileName;
  Proc.Execute;
  Proc.Free;
  // ...
end;
...
begin
  ...
  Project.MakeBinaryIndex         := CreateBinaryIndexCkeckBox.Checked;
...
begin
  ...
  CreateBinaryIndexCkeckBox.Checked := Project.MakeBinaryIndex;
